...
class App extends Component {
  ...
  renderChart = crimeChart => {
    if (crimeChart.labels.length) {
      return (
  ...
  fetchCrimeData = (lat, long, date) => {
    const url = date
      ? `https://data.police.uk/api/crimes-street/all-crime?lat=${lat}&lng=${long}&date=${date}`
  ...
  generateChart = crimeStats => {
    ...
    const categories = crimes.reduce(
      (acc, curr) => {
        acc[0].push(curr[0]);
        acc[1].push(curr[1]);
        return acc;
